...
int main (int argc, char *argv[])
{
  ...
  for(k = 1; k < 6; k++)
  {
    ...
    while (programRuntime < totalRuntime) 
    {
      ...
      time_end_cf = MPI_Wtime();
      time_elapsed_cf += time_end_cf - time_start_cf;

      MPI_Barrier(cart_comm);
      time_start_cv = MPI_Wtime();
      ...
      time_end_cv = MPI_Wtime();
      time_elapsed_cv += time_end_cv - time_start_cv;

      MPI_Barrier(cart_comm);
      time_start_uv = MPI_Wtime();
      ...
      time_end_uv = MPI_Wtime();
      time_elapsed_uv += time_end_uv - time_start_uv;

      MPI_Barrier(cart_comm);
      time_start_bc = MPI_Wtime();
