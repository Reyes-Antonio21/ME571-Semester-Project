...
int main ( int argc, char *argv[] )
{
  ...
  for (k = 1; k < 6; k++)
  {
    ...
    while (programRuntime < totalRuntime)
    {
      ...
      timer_stop(&flux_timer);
      time_elapsed_cf += timer_elapsed(&flux_timer);
      ...
      time_elapsed_cv += timer_elapsed(&compute_timer);
      ...
      time_elapsed_uv += timer_elapsed(&update_timer);
      ...
      timer_stop(&bc_timer);
      time_elapsed_bc += timer_elapsed(&bc_timer);
    }
